...
with tf.name_scope('LstmNet'):
    ...
    states_h, last_h = tf.nn.dynamic_rnn(lstm_cell, x, seq_len, dtype=tf.float32, time_major=True)
    states_h = tf.reshape(states_h, [-1, FLAGS.n_hidden])
    states_h = tf.split(states_h, seq_max_len, 0)
...
with tf.name_scope('dual_attention'):
    ...
    a2 = tf.split(a2, seq_max_len, 0)
    a3 = tf.split(a3, seq_max_len, 0)
    c2 = 0
    c3 = 0
    print("Fuuuu", states_h[0].shape, a2[0].shape)
    for i in range(seq_max_len):
            ...
            for tr in tr_files:
                ...
                for i in range(FLAGS.num_epochs):
                    ...
                    while True:
                        ...
                        batch_cvr, _, batch_loss, batch_cvr_loss, batch_click_loss, batch_eval = sess.run(
                            [cvr, train_op, loss, conversion_loss, loss_click, eval_metric_ops],
                            feed_dict=feed_dict)
                        print(batch_cvr)
                        ...
                        saver.save(sess, os.path.join(FLAGS.model_dir, 'MyModel'), global_step=50)
                        if step % 50 == 0:
                            print("Test----------------")
                            test_cvr_auc = 0.0
                            test_ctr_auc = 0.0
                            ...
                            if test_cvr_auc > best_auc:
                                print("Save----------------")
                                saver.save(sess, os.path.join(FLAGS.model_dir, 'BestModel'))
                            if len(total_label) != FLAGS.batch_size:
                                break
                    tr_infile.close()
        ...
        with tf.Session(config=config) as sess:
            ...
            for te in te_files:
                ...
                while True:
                    ...
                    batch_pcvr, batch_loss, batch_cvr_loss, batch_click_loss, batch_eval = sess.run(
                        [cvr, loss, conversion_loss, loss_click, eval_metric_ops], feed_dict=feed_dict)
                    print(total_seqlen, batch_pcvr, total_label)
                    print("click_AUC = " + "{}".format(batch_eval['CTR_AUC'][0]), end='\t')
                    print("conversion_AUC = " + "{}".format(batch_eval['CTCVR_AUC'][0]), end='\t')
                    print("click_ACC = " + "{}".format(batch_eval['CTR_ACC'][0]), end='\t')
                    print("conversion_ACC = " + "{}".format(batch_eval['CTCVR_ACC'][0]))
