...
tf.flags.DEFINE_integer("user_size", 444862, "Number of Users")
tf.flags.DEFINE_integer("item_size", 4348611, "Number of Items")
...
tf.flags.DEFINE_integer("click_weight", 25, "click_weight")
tf.flags.DEFINE_integer("conversion_weight", 5000, "conversion_weight")
...
def model_fn(features, labels, mode, params):
    ...
    '''epsilon = 1e-7
    click_num = tf.to_float(tf.count_nonzero(click_y))
    conversion_num = tf.to_float(tf.count_nonzero(conversion_y))
    click_weight = (tf.to_float(tf.size(click_y)) - click_num) / (click_num + epsilon)
    conversion_weight = (tf.to_float(tf.size(conversion_y)) - conversion_num) / (conversion_num + epsilon)'''

    ctr_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=y_ctr, labels=y))
    # cvr_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=y_ctcvr, labels=conversion_y))
...
def main(_):
    ...
    tr_files = glob.glob("%s/train/*repeat.tfrecord" % FLAGS.data_dir)
    random.shuffle(tr_files)
    print("train_files:", tr_files)
    te_files = glob.glob("%s/test/*txt.tfrecord" % FLAGS.data_dir)
    ...
    if FLAGS.task_type == 'train':
        ...
        eval_spec = tf.estimator.EvalSpec(
            input_fn=lambda: input_fn(te_files, num_epochs=1, batch_size=FLAGS.batch_size, perform_shuffle=True),
            steps=100,
            start_delay_secs=1000, throttle_secs=1200)
        tf.estimator.train_and_evaluate(Estimator, train_spec, eval_spec)
    elif FLAGS.task_type == 'eval':
        Estimator.evaluate(input_fn=lambda: input_fn(tr_files, num_epochs=1, batch_size=FLAGS.batch_size,perform_shuffle=True), steps=1000)
