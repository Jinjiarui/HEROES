...
class Heroes(tf.keras.layers.Layer):
    def __init__(self, embedding_size, seq_max_len, n_hidden, n_classes, keep_prob, prediction_embed_list):
        ...
        for i in dense_layer_name:
            self.dense_layer[i] = tf.keras.layers.Dense(input_shape=[None, n_hidden], units=n_hidden, use_bias=False,
                                                        kernel_initializer='random_normal', name=i)
        self.drop_out = tf.keras.layers.Dropout(rate=1 - keep_prob)
        self.activate = tf.keras.layers.LeakyReLU()
        self.batch_norm_c = [tf.keras.layers.BatchNormalization() for i in range(len(prediction_embed_list))]
        self.batch_norm_v = [tf.keras.layers.BatchNormalization() for i in range(len(prediction_embed_list))]
        self.prediction_c = [
    ...
    def predict_call(self, inputs, target):
        if target == 'c':
            ...
            batch_norms = self.batch_norm_c
            fc = self.fc_c
        else:
            predict_layers = self.prediction_v
            batch_norms = self.batch_norm_v
            fc = self.fc_v
        for i in range(len(predict_layers)):
            inputs = predict_layers[i](inputs)
            inputs = batch_norms[i](inputs, training=True)
...
class motivate_model(tf.keras.layers.Layer):
    def __init__(self, embedding_size, seq_max_len, n_hidden, n_classes, keep_prob, prediction_embed_list):
        super(motivate_model, self).__init__()
        self.n_hidden = n_hidden
        self.seq_max_len = seq_max_len
    ...
    def call(self, inputs, **kwargs):
        time_scamp = None
        if isinstance(inputs, list):
            inputs, time_scamp = inputs
            time_scamp = tf.transpose(time_scamp, [1, 0, 2])  # (seq,bs,n_class)
        with tf.name_scope('RNN'), tf.variable_scope("RNN", reuse=tf.AUTO_REUSE):
            h_c = tf.zeros(shape=(tf.shape(inputs)[1], self.n_hidden))  # (bs,hidden)
            h_v = tf.zeros(shape=(tf.shape(inputs)[1], self.n_hidden))  # (bs,hidden)
